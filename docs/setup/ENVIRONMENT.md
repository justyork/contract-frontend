...
|------------|-------------|----------|
| `NEXT_PUBLIC_API_URL` | Да | Базовый URL Contractor API, без завершающего слэша (напр. `http://localhost:8080/api` или в проде `https://api.clealex.com/api`). **Указывайте полный домен (включая TLD, напр. `.com`)** — значение подставляется при сборке; после изменения обязательна пересборка и деплой. Используется для REST и редиректов авторизации. |
...
2. **Клиентский бандл:** только `NEXT_PUBLIC_*` подставляются **при сборке**; использовать их только для несекретной конфигурации (URL, publishable keys). **После изменения любой `NEXT_PUBLIC_*` нужно заново собрать и задеплоить приложение** — иначе в бандле остаётся старое значение (например, старый URL для редиректа на Google-авторизацию).
